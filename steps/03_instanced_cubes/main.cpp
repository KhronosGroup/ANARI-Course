...
struct Options
{
  const char *outputFile = "instanced_cubes.png";
...
void makeTranslationMatrix(float x, float y, float z, float m[16])
{
  // clang-format off
  ...
  // clang-format on
  std::memcpy(m, transform, sizeof(transform));
}
...
int main(int argc, const char **argv)
{
  ...
  // Create cube geometry with per-vertex colors.
  // clang-format off
  ...
  ANARIGeometry cubeGeometry = anariNewGeometry(device, "triangle");
  setParameterArray1D(device,
      ...
      8);
  setParameterArray1D(
      device, cubeGeometry, "vertex.color", ANARI_FLOAT32_VEC4, cubeColor, 8);
  setParameterArray1D(device,
      cubeGeometry,
      "primitive.index",
      ANARI_UINT32_VEC3,
      cubeIndex,
      12);
  anariCommitParameters(device, cubeGeometry);
  ...
  ANARIMaterial cubeMaterial = anariNewMaterial(device, "matte");
  ANARIMaterial floorMaterial = anariNewMaterial(device, "matte");
  anariSetParameter(device, cubeMaterial, "color", ANARI_STRING, "color");
  anariCommitParameters(device, cubeMaterial);
  ...
  ANARISurface cubeSurface = anariNewSurface(device);
  ANARISurface floorSurface = anariNewSurface(device);
  anariSetParameter(
      device, cubeSurface, "geometry", ANARI_GEOMETRY, &cubeGeometry);
  anariSetParameter(
      device, cubeSurface, "material", ANARI_MATERIAL, &cubeMaterial);
  anariCommitParameters(device, cubeSurface);
  anariRelease(device, cubeGeometry);
  anariRelease(device, cubeMaterial);
  ...
  // Put the cube surface in a group, then instance that group several times.
  ANARIGroup cubeGroup = anariNewGroup(device);
  setParameterArray1D(
      device, cubeGroup, "surface", ANARI_SURFACE, &cubeSurface, 1);
  anariCommitParameters(device, cubeGroup);
  anariRelease(device, cubeSurface);

  constexpr uint32_t cubeInstanceCount = 5;
  const float cubeTranslation[cubeInstanceCount][3] = {{-5.f, 0.f, 0.f},
      {-2.5f, 0.f, 0.f},
      {0.f, 0.f, 0.f},
      {2.5f, 0.f, 0.f},
      {5.f, 0.f, 0.f}};
  ANARIInstance cubeInstances[cubeInstanceCount] = {};
  for (uint32_t i = 0; i < cubeInstanceCount; ++i) {
    cubeInstances[i] = anariNewInstance(device, "transform");

    float transform[16] = {};
    makeTranslationMatrix(cubeTranslation[i][0],
        cubeTranslation[i][1],
        cubeTranslation[i][2],
        transform);
    anariSetParameter(
        device, cubeInstances[i], "transform", ANARI_FLOAT32_MAT4, transform);
    anariSetParameter(
        device, cubeInstances[i], "group", ANARI_GROUP, &cubeGroup);
    anariCommitParameters(device, cubeInstances[i]);
  }
  anariRelease(device, cubeGroup);

  // Attach the direct floor surface and the cube instances to a world.
  ANARIWorld world = anariNewWorld(device);
  ANARISurface surfaces[] = {floorSurface};
  setParameterArray1D(device, world, "surface", ANARI_SURFACE, surfaces, 1);
  setParameterArray1D(device,
      world,
      "instance",
      ANARI_INSTANCE,
  ...
  for (ANARIInstance instance : cubeInstances)
  ...
  const float position[3] = {5.5f, 3.5f, 7.5f};
