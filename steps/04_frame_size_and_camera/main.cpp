...
struct Options
{
  const char *outputFile = "frame_size_and_camera.png";
...
Options parseOptions(int argc, const char **argv)
{
  ...
  if (argc > 2)
    options.width = std::max(1, std::atoi(argv[2]));
  if (argc > 3)
    options.height = std::max(1, std::atoi(argv[3]));
  if (argc > 4)
    options.libraryName = argv[4];
  if (argc > 5)
    options.deviceName = argv[5];
  return options;
}
...
int main(int argc, const char **argv)
{
  ...
  // Attach both surfaces to a world.
  ...
  const uint32_t frameSize[2] = {options.width, options.height};
